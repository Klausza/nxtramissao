...
const rooms = new Map<string, { host: WebSocket; viewers: Map<WebSocket, string> }>();
...
wss.on('connection', (socket, request) => {
  ...
  socket.on('close', () => { if (!roomCode) return; const room = rooms.get(roomCode); if (!room) return; if (role === 'host') { room.viewers.forEach((_, viewer) => send(viewer, { type: 'host-offline' })); rooms.delete(roomCode); } else { room.viewers.delete(socket); send(room.host, { type: 'viewer-left', count: room.viewers.size }); } });
});
