...
type HostApi = { listSources: () => Promise<Source[]>; openExternal: (url: string) => Promise<void>; publicUrl: () => Promise<string>; startProcessAudio: (handle: number) => Promise<AudioStart>; stopProcessAudio: () => Promise<void>; onAudioChunk: (listener: (chunk: Uint8Array) => void) => () => void };
...
// As salas vivem so na memoria do servidor, entao qualquer restart (deploy, ou o plano
// free do Render hibernando) apaga a sala e o resume-room falha para sempre: a unica
// saida e abrir uma sala nova, com codigo novo.
function handleServerError(msg: any) {
  if (!resuming) return notify(msg.message);
  resuming = false; recreated = true; roomToken = ''; code = '';
  peers.forEach((peer) => peer.close()); peers.clear(); viewerCount.textContent = '0';
  roomCode.textContent = '------';
  notify('A sala anterior expirou porque o servidor reiniciou. Criando uma sala nova...');
  send({ type: 'create-room' });
}

function connectHost(resume: boolean) { resuming = resume; const url = new URL(publicUrl); url.protocol = url.protocol === 'https:' ? 'wss:' : 'ws:'; socket = new WebSocket(url.toString()); socket.onopen = () => send(resume ? { type: 'resume-room', code, token: roomToken } : { type: 'create-room' }); socket.onclose = () => { connectionStatus.textContent = 'RECONECTANDO'; connectionStatus.className = ''; if (code && roomToken) reconnectTimer = setTimeout(() => connectHost(true), 1000); else if (recreated) reconnectTimer = setTimeout(() => connectHost(false), 1000); }; socket.onerror = () => notify('Conexão com o servidor público instável. Tentando reconectar...'); socket.onmessage = async (event) => handle(JSON.parse(event.data)); }
