...
async function handle(msg: Message) { if (msg.type === 'error') return show(msg.message); if (msg.type === 'joined') { const relay = hasRelay(msg.iceServers); peer = new RTCPeerConnection({ iceServers: msg.iceServers }); peer.onicecandidate = (event) => event.candidate && send({ type: 'ice', candidate: event.candidate }); peer.ontrack = (event) => { video.srcObject = event.streams[0]; state.textContent = 'Transmitindo'; }; peer.onconnectionstatechange = () => { if (peer?.connectionState === 'failed') state.textContent = failureText(relay); }; return; } if (msg.type === 'offer' && peer) { await peer.setRemoteDescription(msg.description); const answer = await peer.createAnswer(); await peer.setLocalDescription(answer); send({ type: 'answer', description: peer.localDescription }); } if (msg.type === 'ice' && peer && msg.candidate) await peer.addIceCandidate(msg.candidate); if (msg.type === 'host-offline' || msg.type === 'host-stopped') { state.textContent = 'Transmissão encerrada pelo transmissor'; video.srcObject = null; } }
